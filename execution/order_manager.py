...
# リスクベースのサイジングが効かなくなっていた。docs/DECISIONS.md「検証時の初期資金」の
...
class RestingExitProtection:
    ...
    #
    # **ただし、これを約定検知として当てにしてはならない。** 元になる
    # `reqAllOpenOrdersAsync()` が返すのは板に生きている注文だけで、約定済みの
    # 注文はそもそも含まれない（2026-08-18のINTCで実測。逆指値が97.33で約定した
    # 1分53秒後の照会に、その注文は現れなかった）。約定はローカルの取引ログを
    # 見る `find_filled_resting_exit` でしか読めない。ここが立つのは、約定の
    # 通知が板の更新より先に届いた場合のような限られた並びだけである。
