...
class PositionManager:
    ...
    async def sync_with_broker_async(self, ib: IB) -> None:
        ...
        for broker_position in broker_positions:
            ...
            if existing is None:
                # **WARNINGで出す。** ブローカーが持っている建玉をローカルが
                # 知らない状態は、正常な持ち越しでは起こらない（持ち越しは
                # `logs/positions.json` から復元されるので、ここには来ない）。
                # 来るのは「Botが発注したがローカルへ記録する前に落ちた」か
                # 「運用者が手で建てた」かのどちらかで、**前者はクラッシュの
                # 唯一の痕跡である**（発注カウンタは `placeOrder` の前に
                # 永続化されるが、それだけでは銘柄が分からない）。
                #
                # INFOで出していた頃は、正常な稼働ログに紛れて区別できなかった。
                # 引け後のサマリはWARNINGを種類別に数えるので、上げることで
                # 翌朝に気付ける（`scripts/daily_report`）。
                #
                # **取り込み自体は続ける。** ここで拒否すると、ブローカーが
                # 持っている建玉を誰も決済しない状態になる（待機注文が消えて
                # いれば無防備なまま残る）。危険なのは取り込むことではなく、
                # 取り込んだことに気付かないことである。
                logger.warning(
                    "[%s] ローカルに記録の無い建玉をブローカー側で検出し、追跡を開始します"
                    "（entry=%.2f qty=%s）。**この建玉は次の情報を失っています**: "
                    "建値は手数料込みの avgCost・戦略種別は不明(スイング基準で決済)・"
                    "R倍率の分母なし・同日再エントリーのクールダウンなし・OCAグループ不明。"
                    "Botの発注直後にプロセスが落ちたか、口座で手動の建玉が入ったかのどちらかです。"
                    " `python -m scripts.check_positions` で待機注文の有無を確かめること。",
