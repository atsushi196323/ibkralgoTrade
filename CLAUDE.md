...
**`main.SWING_MARKET_FILTER` の既定はすべてNone（無効）である。** 前2つは同じ軸の符号違いで、どちらが正しいかを事前に決める根拠は無い。`--keep-unfiltered` で「フィルター無し」も候補に残したうえでウォークフォワードに選ばせ、**合算PFだけでなくPFの中央値とプラスで終えた銘柄の割合まで改善したときだけ**ライブで有効化すること。
