...
# 96件/10分で `core.pacing` の実効枠(55件/10分)を超える。ただしこれは 20銘柄でも
# 既に超えていた（80件/10分）ので、24 で新しく生まれた問題ではない。**
...
#     MAX_WATCHLIST_SIZE * (600 / POLL_INTERVAL_SECONDS) <= 60
# を満たす必要がある。24銘柄・300秒なら48件/10分で、日足の初回取得や
# スクリーニングの分の余裕も残る。
#
