...
# 後半7銘柄は2026-08-04に運用者の指示で追加したもので、上の選定基準（bars/の
...
#
# **AMBQ / CBRS / FRVO のティッカーは未確認である**（SPCXは運用者が確認済み）。
# IBKRで qualify できるか、できたとして意図した会社かを
# `python -m scripts.check_market_data --symbol AMBQ` で確認すること。取得できない銘柄は株価帯の判定で監視対象から外れる（WARNING付き）
# ので静かに壊れることは無いが、別会社を掴んでいる場合は気付けない。
#
# SPCX（SpaceX、2026-06-12上場）のティッカーは運用者の確認による。上場から
# 約36営業日で、スイング判定に要る日足30本をようやく満たす程度しか履歴が無い
# （移動平均が全期間をほぼ使い切るため、押し目の基準としては不安定である）。
#
# **これで17銘柄＝上限ちょうどである。** これ以上増やす場合は
# MAX_WATCHLIST_SIZE と POLL_INTERVAL_SECONDS を対で見直すこと。
WATCHLIST: List[str] = [
    "ADBE", "ORCL", "NKE", "INTC", "CRM",
    "UPS", "NVDA", "MRK", "SBUX", "DIS",
    "PLTR", "AMD", "ALAB", "AMBQ", "CBRS", "FRVO", "SPCX",
]
