...
# のは3銘柄まで落ちていた。入れ替えの根拠はdocs/DECISIONS.md「実際に売買している銘柄の
# 根拠」に固定してある。要点だけ:
#
...
# 検証の外側でやることに等しい（docs/DECISIONS.md「複数銘柄での判断」）。この42銘柄は
...
# 戦略なので、戻らなかった銘柄が抜けた母集団は成績を甘く見せる（docs/DECISIONS.md
...
# 検証する方法が無い（PERと同じ制約。docs/DECISIONS.md「銘柄選定」）。
#
...
# （docs/DECISIONS.md「決済の置き場所」）はその分だけ広がる。
#
...
# 遡れず（docs/DECISIONS.md「バックテストのデータ源」）、検証を経ていない条件を
# ライブにだけ入れることになるため。
...
#    60日程度しか遡れないため。docs/DECISIONS.md「バックテストのデータ源」）。MA30の選定も
...
#
# **既定は無効。** 有効化の条件と検証結果は docs/DECISIONS.md「グロース株トラック」節に
...
# シグナルとしての情報が薄い。値はウォークフォワードに選ばせた（docs/DECISIONS.md）。
...
# 「そのうち使うかもしれない」からではない（docs/DECISIONS.md「必要でないもの」）。
...
# どこで切っても一貫性テストの結論が変わらなかった（`docs/DECISIONS.md`「母集団を
# 絞ってもエッジは残る」）ので、**運用条件で決めてよい**軸である。
#
# **運用者の指定により売買代金の上位500銘柄。** 2026-08-27の実測では、
# 100〜500のどこで切っても一貫性テスト8項目がすべて合格する（年率超過 対数
# +8.9〜+27.0%）。**サイズを成績で選んではならない**——読むべきは
# 「どこで切っても結論が変わらない」という向きだけである。
#
# **20から500へ戻した経緯を残す。** 一時的に20を指定していたが、20銘柄では
# 上位10%が2銘柄になり、**8項目で最も強い「順位の単調性」が測定不能**になる
# （バケット平均の下限5銘柄に届かない）。不合格ではなく測れない状態で、
# 「測って良かった構成」とは言えなかった。
MOMENTUM_UNIVERSE_SIZE: int = 500
...
# 母集団に残すための、1日あたりの売買代金の下限(USD)。
#
# **上位N件に絞ることとは別の歯止めである。** 2026-08-06に627銘柄で層別した
# 測定では、押し目買いの成績は 1–100位 PF 1.26 / 201–400位 1.30 に対し
# **401位以下だけが 1.10** と明確に悪かった。**効くのは流動性の下限を切ることで
# あって、上位に絞ることではない。** しかもこの測定はスプレッドの差を含んで
# いないので、実際の下位はこの数字よりさらに悪い。
#
# 上位500件の枠に入っていても、相場が薄い日には売買代金が落ちる。件数の枠だけ
# だと、その日たまたま500位以内に入った細い銘柄が母集団に混じる。
#
# 2,000万ドル/日は、¥500,000の1建玉($628)が板に与える影響が無視できる水準として
# 置いた**運用上の値であり、成績で選んだ値ではない**。
MOMENTUM_MIN_TURNOVER_USD: float = 20_000_000.0
...
MOMENTUM_CONFIG: MomentumConfig = MomentumConfig(
    ...
    # 薄い母集団の中の順位は上位10%を意味しない、という歯止め。
    # **`MOMENTUM_UNIVERSE_SIZE` を下げるときにここも下げてはならない**——
    # 下げると歯止めそのものが無くなる。届かない日は建てないのが正しい。
    min_symbols=MIN_SYMBOLS_FOR_RANKING,
)
...
# （docs/DECISIONS.md「決済の置き場所」）は、幅を広げても失われない。
...
async def resolve_momentum_targets_async(
    ...
    **価格が取れない銘柄は母集団から落ちる。** 落ちた結果 `min_symbols` を
    ...
    if MOMENTUM_UNIVERSE_SIZE > 0:
        ...
        kept = set(select_by_turnover(
            turnovers, MOMENTUM_UNIVERSE_SIZE, MOMENTUM_MIN_TURNOVER_USD,
        ))
...
# 参照価格そのものがずれているケースは検出できない（docs/DECISIONS.md該当節）。
#
...
async def _get_market_deviation_pct_async(
    ...
    「1取引日あたり1件」で済み、ペーシング制限(docs/DECISIONS.md 6.1)には実質響かない。
    """
...
def resolve_min_tradeable_price(
    ...
    リスクが1.00% -> 0.29% になっていた（docs/DECISIONS.md「検証時の初期資金」節）。
