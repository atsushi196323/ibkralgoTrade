...
class DailyBarCache:
    ...
    # 既定の取得期間。**取得は取引日ごとに銘柄あたり1回だけなので、期間を
    # 延ばしてもペーシング制限(§6.1)上のリクエスト数は増えない。**
    #
    # 暦日300日（≒207営業日）は、スイング判定の移動平均(main.SWING_MA_WINDOW =
    # 30本)に十分な余裕を持たせつつ、**長期トレンド判定の200日移動平均を
    # 同じバーから計算できる**長さとして選んでいる。以前は90日だったが、
    # ウォッチリストから下降トレンドの銘柄を落とす判定
    # (main.STRUGGLING_MA_WINDOW)のために別途300日を取り直すと、
    # 銘柄あたりのリクエストが1日2回に倍増していた。
    #
    # 本数が足りないとmain側の `len(daily_df) >= SWING_MA_WINDOW` で
    # 日足分岐がスキップされる（WARNINGは出る）。
    def __init__(self, duration: str = "300 D") -> None:
